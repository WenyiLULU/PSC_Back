...
fileUploader = require("../config/cloudinary.config")
...
// upload image
router.post('/:userId/image', fileUploader.single("userPhoto"), async (req, res, next) => {
  const { userId }= req.params
...
try {
  ...
  res
    .status(201)
    .json({ message: "User photo updated", name: user.username });
} catch (error) {
  res.status(500).json(error);
}
});
